...
export class MemoriaController {
    ...
    constructor(private memoryService: MemoriaService) {}

    @Get('/')
    async getAllOthers(@Req() req, @Res() res) {
      const otros = await this.memoryService.getAllMemorys();
    ...
    async createOthers(@Req() req, @Res() res, @Body() memory: any) {
      try {
        ...
        const memoryCreated =  await this.memoryService.createMemoria( payload )
        ...
        return res.status(400).json({
          ok: false,
          error: {
            message: 'No se pudo agregar a la base de datos la memoria ingresado',
    ...
    @Put('/update/:id')
    async updateOthers(@Req() req, @Res() res, @Body() memory: any, @Param('id') id: string) {
      // console.log({req, res})
      try {

        const memoryUpdate = await this.memoryService.updateMemory(id, memory);
  ...
  async deleteOthers(@Req() req, @Res() res, @Param('id') id: string) {
    // console.log({req, res})
    try {
      const memoryDeleted = await this.memoryService.deleteMemory(id);
      if (!memoryDeleted) {
     ...
     async uploadoImage(@Req() req, @Res() res: Response, @Body() user: any, @Param('id') id: string, @Param('tipoImagen') tipoImagen: string ) {
       try {
         ...
         const path = `/home/muho/Documents/nestJs/invertario/dist/memoria/uploads/${tipoImagen}/${nombreImagenPersonalizado}`;
         ...
         const otrosUpdate: any = await this.memoryService.subirImagenPorTipo( tipoImagen, id, nombreImagenPersonalizado,res);
      //    return res.json({
      //      ok: true,
      //      path
      //    })
